...
const OrderStatusDonut = () => {
  ...
  const chartOptions = {
    chart: { type: 'donut' },
    labels,
    colors: labels.map(l => ORDER_COLORS[l] || '#94a3b8'),
    ...
    const fetchData = async () => {
      try {
        const { data } = await axios.get(
          `${backendUrl}/api/dashboard/order-status-chart`,
        ...
        console.error('Failed to fetch order status chart:', error);
      }
    };
    fetchData();
  }, [backendUrl]);
    ...
    <Col xs={12} lg={4}>
      <Card className="shadow h-100">
        <CardHeader className="p-4 border-bottom">
          <h5 className="card-header-title mb-0">Order Status Distribution</h5>
        </CardHeader>
