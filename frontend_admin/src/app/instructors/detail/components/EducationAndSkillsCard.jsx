...
const EducationAndSkillsCard = ({ educationList = [], skillsList = [] }) => {
  ...
  return (
    <Card className="card-body shadow p-4">
      ...
      {edList.length === 0 ? (
        <div className="mt-2">No education information available.</div>
      ) : (
        edList.map((edu, idx) => (
          <div key={idx} className="d-flex align-items-center mt-3">
            <span
      ...
      {skList.length === 0 ? (
        <div className="mt-2">No skills available.</div>
      ) : (() => {
        ...
        return (
          <>
            {shown.map((skill, idx) => {
              ...
              return (
                <div key={idx} className="overflow-hidden mt-3">
                  <div className="d-flex align-items-end justify-content-between">
                    <h6 className="mb-1 text-break">{skill.name || "Unknown"}</h6>
                    <span className="progress-percent-simple">{level}%</span>
                  </div>
            ...
            {remaining > 0 && (
              <div className="small mt-3">and <strong>{remaining}</strong> more.</div>
            )}
          </>
        );
      })()}
