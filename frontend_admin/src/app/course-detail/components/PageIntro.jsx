...
const PageIntro = ({ course }) => {
  const averageRating =
    course?.rating?.count > 0
      ? (course?.rating?.total || 0) / course.rating.count
      : 0;

  return (
    <section
      className="bg-blue py-0 py-sm-4 text-white"
      style={{
        ...
        <Row className="py-4">
          <Col lg={8}>
            <div className="mb-3 d-flex flex-column flex-sm-row align-items-start gap-2">
              ...
              {course?.status && (
                <h6 className="mb-0 bg-dark bg-opacity-25 text-white py-2 px-3 rounded-2 d-inline-block text-capitalize">
                  {course.status}
                </h6>
              )}
            </div>
            ...
            <ul className="list-inline mb-0">
              ...
              <li className="list-inline-item h6 me-3 mb-1 icons-center text-white">
                <MdLabel
                  size={16}
                  className="text-purple me-2 mb flex-shrink-0"
                />
