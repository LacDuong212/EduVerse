...
const CourseDetail = () => {
    ...
    const fetchCourse = async () => {
      try {
        ...
        if (data.success) {
          ...
          // Ghi nhận viewed (không block UI)
          await axios.post(
  ...
  return (
    <div>
     {/* Banner */}
    <CourseHeaderBanner course={course} />   {/* thay thế phần banner cũ */}
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {course.sections?.length > 0 ? (
              <div className="space-y-3">
                {course.sections.map((section, idx) => (
                  <details key={idx} className="bg-gray-50 border rounded-lg p-4">
                    <summary className="cursor-pointer font-medium">
                      {section.title}
                    </summary>
                    <ul className="ml-5 mt-2 list-disc text-gray-600 text-sm space-y-1">
        ...
        {/* Sidebar (preview video stays here as you requested) */}
        <div className="lg:col-span-1">
          <div className="sticky top-20 bg-white rounded-xl shadow-lg p-6 space-y-4">
            {/* Preview video or thumbnail (in sidebar) */}
            ...
            <button
              ...
              className="w-full border border-gray-300 bg-white text-black py-2.5 rounded-lg font-medium hover:bg-gray-100 transition"
            >
              Add To Cart
            </button>
