        ...
        <div className="table-responsive border-0">
          <table className="table table-dark-gray align-middle mb-0 table-hover">
            <thead className={`border-0 border-start border-${HIGHLIGHT_THICKNESS} border-dark`}>
              <tr>
                <th className="border-0 p-0 rounded-0"></th>
            ...
            <tbody>
              {loading ? (
                ...
                const hasChanges = course?.hasChanges ?? false;
                const underReview = course?.status === "pending" || course?.requestUpdate;

                const highlight = underReview ? "warning" : hasChanges ? "info" : "";
                const tooltipMsg = underReview ? "Submitted & Under Review" : hasChanges ? "Has Ongoing Changes" : "?";

                return (
                  <tr key={course?.courseId || idx} className="position-relative">
                    <td className={highlight ? `border-0 border-start border-${highlight} border-${HIGHLIGHT_THICKNESS} p-0` : "p-0"}>
                      {highlight && (
                        <OverlayTrigger
                          placement="bottom"
