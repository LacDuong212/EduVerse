...
// Hàm helper để map dữ liệu từ API sang format của Card
const adaptCourseData = (c) => {
  ...
  const instructorName = c?.instructor?.name ?? c?.instructor?.username ?? 'Unknown';
  ...
  return {
    ...
    category: c?.category?.name || (typeof c?.category === 'string' ? c.category : 'Others'),
    _raw: c,
  };
};
...
const CommonCourseSlider = ({ source, courses }) => {
  ...
  if (courses && Array.isArray(courses)) {
      ...
      rawList = Array.isArray(coursesState[source]) ? coursesState[source] : [];
  }
